...
function obtenerPagos() {
    ...
    if ($db) {
        try {
            $sql = "SELECT TOP 60
                p.CFNUMPED as id,
                p.CFNUMPED as nro_pedido,
                COALESCE(NULLIF(LTRIM(RTRIM(p.CFRFNUMDOC)), ''), LTRIM(RTRIM(p.CFNUMPED))) as nro_factura,
                CONVERT(varchar, p.CFFECDOC, 23) as fecha,
                LTRIM(RTRIM(p.CFNOMBRE)) as cliente,
                LTRIM(RTRIM(p.CFRUC)) as ruc,
                CAST(p.CFIMPORTE as float) as monto,
                LTRIM(RTRIM(p.CFUSER)) as asesor,
                LTRIM(RTRIM(p.CFBANCO)) as banco_codigo,
                LTRIM(RTRIM(p.CFOPERP)) as nro_operacion,
                CAST(p.CFMONTOP as float) as monto_pagado,
                CONVERT(varchar, p.CFFECHAP, 23) as fecha_pago,
                CASE 
                    WHEN p.CFESTADO = '1' THEN 'Pendiente'
                    WHEN p.CFESTADO = '2' THEN 'Aceptado'
                    ELSE 'Pendiente'
                END as estado
            FROM PEDCAB p
            ...
            if ($stmt) {
                ...
                if (!empty($rows)) {
                    ...
                    foreach ($rows as $idx => $r) {
                        $bancoName = 'BCP';
                        if ($r['banco_codigo'] === '02' || stripos($r['banco_codigo'], 'BBVA') !== false) {
                            $bancoName = 'BBVA';
                        } elseif ($r['banco_codigo'] === '03' || stripos($r['banco_codigo'], 'Inter') !== false) {
                            $bancoName = 'Interbank';
                        }

                        $pagos[] = [
                            'id' => $idx + 1,
                            'nro_factura' => 'PED-' . $r['nro_pedido'],
                            'cliente' => $r['cliente'],
                            'ruc' => $r['ruc'],
                            'monto' => round($r['monto'], 2),
                            'metodo' => 'Transferencia ' . $bancoName,
                            'nro_operacion' => !empty($r['nro_operacion']) ? ($bancoName . ' Op. #' . $r['nro_operacion']) : ('Op. Pendiente #' . $r['nro_pedido']),
                            'banco' => $bancoName,
                            'asesor' => ucfirst(strtolower($r['asesor'] ?: 'Endrina')),
                            'fecha' => $r['fecha'] . ' 12:00:00',
                            'voucher_url' => 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=600&q=80',
                            'estado' => $r['estado'],
                            'validador' => $r['estado'] === 'Aceptado' ? 'Nayeli (Reportería)' : null,
                            'fecha_validacion' => $r['estado'] === 'Aceptado' ? $r['fecha_pago'] : null,
                            'motivo_observacion' => null
                        ];
                    }
    ...
    if ($db) {
        try {
            $sql = "SELECT TOP 60 
                ...
                LTRIM(RTRIM(c.CCRUC)) as ruc_dni,
                LTRIM(RTRIM(c.CCDIRECC)) as direccion,
                LTRIM(RTRIM(c.CCUSER)) as asesor,
                CAST(c.CCIMPORTE as float) as total,
                CAST(c.CCIGV as float) as igv,
                CAST(c.CCDESVAL as float) as subtotal,
                CASE 
                    WHEN c.CCESTADO = '1' THEN 'Emitida'
                    WHEN c.CCESTADO = '2' THEN 'Aprobada'
                    WHEN c.CCESTADO = '3' THEN 'Facturada'
                    WHEN c.CCESTADO = 'A' THEN 'Anulada'
                    ELSE 'Emitida'
                END as estado
            FROM COTCAB c 
                ...
                if (!empty($rows)) {
                    ...
                    foreach ($rows as $idx => $r) {
                        $cots[] = [
                            ...
                            'forma_pago' => 'CONTADO',
                            'vigencia' => '7 dias',
                            'subtotal' => round($r['subtotal'] ?: ($r['total'] / 1.18), 2),
                            'igv' => round($r['igv'] ?: ($r['total'] - ($r['total'] / 1.18)), 2),
                            'total' => round($r['total'], 2),
                            'estado' => $r['estado'],
                            'items' => [],
                            'created_at' => $r['fecha'] . ' 10:00:00'
                        ];
                    }
    ...
    if ($db) {
        try {
            ...
            $sql = "SELECT 
                ...
                CAST(SUM(p.CFIMPORTE) as float) as monto_acumulado
            FROM PEDCAB p
            WHERE CONVERT(varchar, p.CFFECDOC, 23) >= DATEADD(day, -7, GETDATE())
            GROUP BY p.CFUSER
            ...
            if ($stmt) {
                ...
                if (!empty($rows)) {
                    $cierres = [];
                    foreach ($rows as $idx => $r) {
                        $asesorName = ucfirst(strtolower($r['asesor'] ?: 'Asesor'));
                        $cierres[] = [
                            'id' => $idx + 1,
                            'asesor' => $asesorName,
                            'sucursal' => ($asesorName === 'Endrina' ? 'Sucursal Chorrillos' : 'Sede Principal'),
                            'fecha' => $hoy,
                            'hora' => date('H:i'),
                            'total_ventas' => intval($r['total_ventas']),
                            'monto_acumulado' => round($r['monto_acumulado'], 2),
                            'nota' => "Cierre consolidado Starsoft ERP ({$r['total_ventas']} pedidos procesados).",
                            'estado' => 'Aprobado',
                            'validador' => 'Nayeli (Reportería)',
    ...
    if ($action === 'obtener_detalle_factura_completa') {
        ...
        try {
            ...
            $sql = "SELECT TOP 1 XML, CFNUMSER, CFNUMDOC, RUC_EMISOR, TIPODOC_COMPROBANTE, 
                           ...
                           LTRIM(RTRIM(COALESCE(TIPO_PAGO, 'CONTADO CONTRA ENTREGA'))) as forma_pago,
            ...
            $cabeceraData = [
                ...
                'fecha_vcto_dmy' => ($detalleXml['cuotas'][0]['fecha_vcto'] ?? '') ?: ($row['fecha_dmy'] ?? ''),
                'razon_social' => $row['razon_social'],
                'ruc' => $row['ruc'],
                'direccion' => $row['direccion_cliente'] ?? '',
                'forma_pago' => $row['forma_pago'] ?? 'CONTADO CONTRA ENTREGA',
                'nro_pedido' => ($detalleXml['documento']['nro_pedido'] ?? '') ?: '-',
                'orden_compra' => ($detalleXml['documento']['orden_compra'] ?? '') ?: ($row['orden_compra'] ?? '-'),
                'cod_vendedor' => '07 CARMEN LOLOY',
